...
rotational_speed = np.where(
    speed_mode == 0, np.random.normal(1500, 100, N_ROWS), np.random.normal(2500, 150, N_ROWS)
)
...
osf_prob = np.array(
    [0.1 if overstrain[i] > osf_threshold[product_types[i]] else 0.001 for i in range(N_ROWS)]
)
...
edge_proc += np.random.exponential(1.5, N_ROWS) + pre_failure_window * np.random.uniform(
    5, 18, N_ROWS
)
...
http_5xx = np.clip(
    np.round(error_rate / 100 * request_volume + np.random.poisson(1, N_ROWS)), 0, 120
).astype(int)
...
sw_only_mask = (
    (machine_failure == 0) & (pre_failure_window == 0) & (np.random.random(N_ROWS) < 0.025)
)
