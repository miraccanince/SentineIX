    ...
    try:
        with engine.connect() as conn:
            ...
            high_risk = conn.execute(text(
                "SELECT COUNT(*) FROM alerts WHERE risk_level IN ('high', 'critical')"
            )).scalar() or 0
            acknowledged = conn.execute(text(
                "SELECT COUNT(*) FROM alerts WHERE acknowledged = true"
            )).scalar() or 0
...
def calculate_savings(alerts_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if len(labeled) > 0:
        fp_count = len(labeled[labeled['false_positive'] == True])
        tp_count = len(labeled[labeled['false_positive'] == False])
    ...
    industry_savings = (industry_tp_at_scale * NET_SAVINGS_PER_TP) - (industry_fp_at_scale * COST_PER_FALSE_POSITIVE)
...
def calculate_projected_savings() -> Dict[str, Any]:
    ...
    sentinelx_savings = (sentinelx_tp * NET_SAVINGS_PER_TP) - (sentinelx_fp * COST_PER_FALSE_POSITIVE)
...
def create_fp_comparison_chart() -> go.Figure:
    ...
    fig.add_trace(go.Bar(
        ...
        textfont=dict(size=24, color='#333')
    ))
    ...
    fig.update_layout(
        title={
            'text': f'Alert Fatigue Reduction: {reduction:.0f}%',
            'font': {'size': 20}
        },
...
def create_root_cause_pie(df: pd.DataFrame) -> go.Figure:
    ...
    if df.empty:
        # Demo data
        df = pd.DataFrame({
            'root_cause': ['mechanical_wear', 'software_stress', 'thermal_overload',
                          'network_congestion', 'power_anomaly'],
            'count': [35, 28, 18, 12, 7]
        })
...
def main():
    ...
    # Header
    st.markdown('<p class="main-header">🛡️ SentinelX Predictive Maintenance</p>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">AI-Powered Industrial Intelligence | Real-Time Value Tracking</p>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        refresh_rate = st.selectbox("Auto-refresh", ["Off", "30s", "1min", "5min"], index=0)
    ...
    with col1:
        ...
        fig = create_savings_gauge(
            savings_data['annual_projection'],
    ...
    if not alerts_df.empty:
        ...
        if not high_risk.empty:
            display_df = high_risk[['timestamp', 'machine_id', 'risk_level', 'root_cause', 'failure_prob', 'acknowledged']]
            display_df.columns = ['Timestamp', 'Machine', 'Risk', 'Root Cause', 'Probability', 'Acknowledged']
            st.dataframe(display_df, use_container_width=True)
        else:
            st.info("No high-risk alerts in the selected time range.")
    else:
        st.info("No alert data available. Run predictions through the API to populate this dashboard.")
    ...
    with st.expander("Submit Test Prediction"):
        ...
        scenarios = {
            'normal': {
                'air_temp': 305.0, 'torque': 40.0, 'tool_wear': 100.0, 'vibration': 18.0,
                'error_rate': 2.0, 'cpu_util': 55.0, 'disk_io': 8.0, 'http_5xx': 0,
                'queue_depth': 15, 'api_latency': 120.0, 'network_latency': 20.0
            },
            'stress': {
                'air_temp': 330.0, 'torque': 70.0, 'tool_wear': 200.0, 'vibration': 35.0,
                'error_rate': 15.0, 'cpu_util': 88.0, 'disk_io': 30.0, 'http_5xx': 20,
                'queue_depth': 100, 'api_latency': 400.0, 'network_latency': 60.0
            },
            'failure': {
                'air_temp': 365.0, 'torque': 100.0, 'tool_wear': 280.0, 'vibration': 48.0,
                'error_rate': 28.0, 'cpu_util': 98.0, 'disk_io': 48.0, 'http_5xx': 80,
                'queue_depth': 400, 'api_latency': 750.0, 'network_latency': 120.0
            }
        }
        ...
        if st.button("🔮 Predict", type="primary"):
            payload = {
                ...
                "application": {
                    ...
                    "request_throughput_rps": max(10, 150 - queue_depth * 0.3)  # Throughput drops with queue
                }
            }

            # Debug: Show actual values being sent
            st.markdown("**📋 Debug - Values Sent to API:**")
            st.code(f"air_temp={air_temp}, torque={torque}, tool_wear={tool_wear}, vibration={vibration}\n"
